# v2.5 - Used chatgpt to finally solve TBC arrow slide in animation
...
def start():
	...
	# TBC arrow slide in (written using ChatGPT help)
	...
	arrowstart = (vidwid*0.9,vidhei*0.9)
	arrowend = (vidwid*0.1,vidhei*0.9)

	# Define the animation as a list of (time, position) tuples
	animation = [(0, arrowstart), (1, arrowend)]

	# Animate the clip by setting its position at each keyframe
	def animate(clip, t):
		x, y = animation[t][1]
		return tbcarrow.set_position((x, y))

	arrowslide = tbcarrow.fl(animate, apply_to=['position'])

	# tried multiple methods that ChatGPT generated, but they didnt work, and each time it said sorry, and generated another incorrect program
...
## JoJoTBCfy
startbutton = Button(master, text='JoJoTBCfy!', command=lambda:threading.Thread(target=start).start(), width=20, height=2, bg='#1d1c2c', fg='#8d73ff', activebackground='#1d1c2c' , activeforeground='#8d73ff')
